...
namespace CocaineCrackDown.Scener {
    public class VärdScen : GrundScen {
        ...
        private void TextFält(Button obj) {
            ...
            Core.StartSceneTransition(new TextureWipeTransition(() => new NätLobby(true){Server = ServerHanterare}) { TransitionTexture = Core.Content.Load<Texture2D>("nez/textures/textureWipeTransition/wink") });
        }
    }
}
