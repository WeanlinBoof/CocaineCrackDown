...
namespace RedGrin
{
    public class NetworkManager {
        ...
        private readonly Dictionary<byte , NetConnection> clientIdConnectionMap = new Dictionary<byte , NetConnection>();
        ...
        public static NetworkManager Self => self ??= new NetworkManager();
        ...
        /// </summary>
        public long NetworkId => network == null
        ...
        public double ServerTime {
            get {
                ...
                else if(network?.Connections?.Count > 0) {
                    return network.Connections[0].GetRemoteTime(NetTime.Now);
                }
                else {
                    return -1;
                }
        ...
        public bool IsConnected => Role == NetworkRole.Server || network.ConnectionsCount > 0;
        ...
        public void Start(NetworkRole role) {
            ...
            NetPeerConfiguration config = new NetPeerConfiguration(Configuration.ApplicationName);
        ...
        public void Disconnect() {
            const string msg = "Disconnecting...";
            log.Info(msg);
            foreach(NetConnection connection in network.Connections) {
        ...
        public void DeadReckon() {
            ...
            for(int i = 0; i < entities.Count; i++) {
                ...
                if(Role == NetworkRole.Server || (Role == NetworkRole.Client && entities[i].IsOwned())) {
        ...
        {
            NetConnection connection = network.Connections.Where(c => c.RemoteUniqueIdentifier == id).FirstOrDefault();
            return clientIdConnectionMap.FirstOrDefault(kvp => kvp.Value == connection).Key;
        }
    }
}
